...
public class RequestClearTable implements IRequest {
	...
	public HttpServletResponse process(RequestInfo request, HttpServletResponse response) throws IOException, ServletException {
		...
		LOG.trace("Employee #{} is requesting to clear Table #{}...", employee.getId(), id);
		
		try {
			// attempt to load the table
			table = new Table(id);
			table.clearTable(employee);
	...
	public String getCommand() { return "clear"; }
}